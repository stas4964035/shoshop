from django.shortcuts import render
from django.shortcuts import get_object_or_404
from django.contrib.auth.decorators import user_passes_test
from django.shortcuts import HttpResponseRedirect
from django.urls import reverse
from django.urls import reverse_lazy
from django.views.generic.list import ListView
from django.views.generic.edit import CreateView
from django.views.generic.edit import UpdateView
from django.views.generic.edit import DeleteView
from django.views.generic.detail import DetailView
from django.utils.decorators import method_decorator
from mainapp.models import Deal
from mainapp.models import ListOfCountries
from authapp.models import DealUser
from authapp.forms import DealUserRegisterForm
from adminapp.forms import DealUserAdminEditForm
from adminapp.forms import DealEditForm


# админка - список пользователей
class DealUsersListView(ListView):
    model = DealUser
    template_name = 'adminapp/users.html'

    @method_decorator(user_passes_test(lambda u: u.is_superuser))
    def dispatch(self, *args, **kwargs):
        return super().dispatch(*args, **kwargs)


# админка - создание пользователя
@user_passes_test(lambda u: u.is_superuser)
def user_create(request):
    title = 'пользователи/создание'

    if request.method == 'POST':
        user_form = DealUserRegisterForm(request.POST, request.FILES)
        if user_form.is_valid():
            user_form.save()
            return HttpResponseRedirect(reverse('admin:users'))
    else:
        user_form = DealUserRegisterForm()

    content = {
        'title': title,
        'update_form': user_form,
    }

    return render(request, 'adminapp/user_update.html', content)


# админка - редактирование пользователя
@user_passes_test(lambda u: u.is_superuser)
def user_update(request, pk):
    title = 'пользователи/редактирование'

    edit_user = get_object_or_404(DealUser, pk=pk)

    if request.method == 'POST':
        edit_form = DealUserAdminEditForm(request.POST,
                                            request.FILES,
                                            instance=edit_user)
        if edit_form.is_valid():
            edit_form.save()
            return HttpResponseRedirect(reverse('admin:user_update',
                                                args=[edit_user.pk]))
    else:
        edit_form = DealUserAdminEditForm(instance=edit_user)

    content = {
        'title': title,
        'update_form': edit_form,
    }

    return render(request, 'adminapp/user_update.html', content)


# админка - удаление пользователя
@user_passes_test(lambda u: u.is_superuser)
def user_delete(request, pk):
    title = 'пользователи/удаление'

    user = get_object_or_404(DealUser, pk=pk)

    if request.method == 'POST':
        user.is_active = False
        user.save()
        return HttpResponseRedirect(reverse('admin:users'))

    content = {
        'title': title,
        'user_to_delete': user,
    }

    return render(request, 'adminapp/user_delete.html', content)


# админка - список стран
@user_passes_test(lambda u: u.is_superuser)
def countries(request):
    title = 'админка/страны'

    countries_list = ListOfCountries.objects.all()

    content = {
        'title': title,
        'objects': countries_list
    }

    return render(request, 'adminapp/countries.html', content)


# админка - создание страны
class CountryCreateView(CreateView):
    model = ListOfCountries
    template_name = 'adminapp/country_update.html'
    success_url = reverse_lazy('admin:countries')
    fields = '__all__'


# админка - редактирование страны
class CountryUpdateView(UpdateView):
    model = ListOfCountries
    template_name = 'adminapp/country_update.html'
    success_url = reverse_lazy('admin:countries')
    fields = '__all__'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = 'страны/редактирование'

        return context


# админка - удаление страны
class CountryDeleteView(DeleteView):
    model = ListOfCountries
    template_name = 'adminapp/country_delete.html'
    success_url = reverse_lazy('admin:countries')

    def delete(self, request, *args, **kwargs):
        self.object = self.get_object()
        self.object.is_active = False
        self.object.save()

        return HttpResponseRedirect(self.get_success_url())


# админка - список предложений компании
@user_passes_test(lambda u: u.is_superuser)
def deals(request, pk):
    title = 'админка/размещение'

    country = get_object_or_404(ListOfCountries, pk=pk)
    deals_list = Deal.objects.filter(
        country__id=pk).order_by('name')

    content = {
        'title': title,
        'country': country,
        'objects': deals_list,
    }

    return render(request, 'adminapp/deals.html', content)


# админка - создание нового предложения
@user_passes_test(lambda u: u.is_superuser)
def deal_create(request, pk):
    title = 'предложения/создание'
    country = get_object_or_404(ListOfCountries, pk=pk)

    if request.method == 'POST':
        pass
        deal_form = DealEditForm(request.POST, request.FILES)
        if deal_form.is_valid():
            deal_form.save()
            return HttpResponseRedirect(reverse('admin:deals',
                                                args=[pk]))

    else:

        deal_form = DealEditForm(
            initial={'country': country})
    content = {
        'title': title,
        'update_form': deal_form,
        'country': country,
    }
    return render(request, 'adminapp/deal_update.html', content)


# админка - редактирование предложения
@user_passes_test(lambda u: u.is_superuser)
def deal_update(request, pk):
    title = 'размещение/редактирование'
    edit_deal = get_object_or_404(Deal, pk=pk)

    if request.method == 'POST':
        deal_edit_form = DealEditForm(
            request.POST, request.FILES, instance=edit_deal)
        if deal_edit_form.is_valid():
            deal_edit_form.save()
            return HttpResponseRedirect(
                reverse('admin:deal_update',
                        args=[edit_deal.pk]))
    else:
        deal_edit_form = DealEditForm(
            instance=edit_deal)
    content = {
        'title': title,
        'update_form': deal_edit_form,
        'country': edit_deal.country,
    }
    return render(request, 'adminapp/deal_update.html', content)


# админка - карточка предложения компании
class DealDetailView(DetailView):
    model = Deal
    template_name = 'adminapp/deal_read.html'


# админка - удаление предложения
@user_passes_test(lambda u: u.is_superuser)
def deal_delete(request, pk):
    title = 'размещение/удаление'
    deal = get_object_or_404(Deal, pk=pk)

    if request.method == 'POST':
        deal.is_active = False
        deal.save()
        return HttpResponseRedirect(reverse('admin:deals',
                                            args=[deal.country.pk]))
    content = {
        'title': title,
        'deal_to_delete': deal,
    }
    return render(request, 'adminapp/deal_delete.html', content)
